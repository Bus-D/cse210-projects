public class Reference
{
    ...
    public void GetReference()
    {
        ...
        {
            Console.WriteLine("Please enter the book of the scripture you want to memorize. (ex. 1 Nephi)");
            Console.Write("> ");
            string _chapRef = Console.ReadLine();
            Console.WriteLine("");
            bool _hasChapter = _chapRef.Split(" ").Length > 1;

            if (_hasChapter)
            {
                string[] _refSplit = _chapRef.Split(" ");
                Chapter = int.Parse(_refSplit[0]);
                Book = _refSplit[1];
            }
            ...
            Console.WriteLine("");
            bool IsAll = _verse.Split("-").Length > 1;
    ...
    public void IsReady()
    {
        ...
        while (!_isReady)
        {
            if (_isReady)
            {
                Console.WriteLine($"The reference is: {Chapter} {Book}: {VerseStart}-{VerseEnd}");
            ...
            {
                Console.WriteLine($"The reference is: {Chapter} {Book}: {VerseStart}");
                Console.WriteLine("Is this correct? Y/N");
                string _correct = Console.ReadLine();
                string _isCorrect = _correct.Trim().ToLower();
